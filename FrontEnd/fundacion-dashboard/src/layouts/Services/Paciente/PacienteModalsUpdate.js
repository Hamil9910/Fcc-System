...
function PacienteModalsUpdate(idPaciente,{  onClose }) {  
  ...
  useLayoutEffect(() => {
    const cargarDatosPaciente = async () => {
      if (idPaciente) {
        try {
          const respuesta = await axios.get(`http://localhost:5000/api/fcc/paciente/${idPaciente.idPaciente}`);
          // Suponiendo que la respuesta tiene la forma { nombre_enfermedad: ..., ... }
          setFormData(respuesta.data);
        } catch (error) {
          console.error("Error al paciente", error);
        }
      }
    };
    cargarDatosPaciente()
  ...
  const enviarDatos = async () => {
    ...
    try {
      await axios
        .put(`http://localhost:5000/api/fcc/paciente/${idPaciente.idPaciente}`, dataParaEnviar)
      ...
      Swal.fire({
        title: "Error",
        text: "Hubo un problema al modificar al Paciente",
        icon: "error",
        confirmButtonText: "Ok",
      }).then((result) => {
        // Verifica si el botón "Ok" fue presionado
...
PacienteModalsUpdate.propTypes = {
  onClose: PropTypes.func.isRequired,
};

export default PacienteModalsUpdate;
