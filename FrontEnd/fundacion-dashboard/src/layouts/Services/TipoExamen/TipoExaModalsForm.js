...
function TipoExaModalsForm({ onClose }) {
  ...
  const enviarDatos = async () => {
    ...
    const dataParaEnviar = {
      id_tipo_examen : generarIdUnico(), // Genera un ID único
      nombre_tipo_examen: formData.nombre_tipo_examen,
    };
    try {
      await axios
        .post("http://localhost:5000/api/fcc/tipo_examen", dataParaEnviar)
      // Muestra un mensaje de éxito
      Swal.fire({
        title: "¡Éxito!",
        text: "Tipo de Examen agregado correctamente",
        icon: "success",
      ...
      Swal.fire({
        title: "Error",
        text: "Hubo un problema al agregar el Tipo de Examen",
        icon: "error",
        confirmButtonText: "Ok",
      }).then((result) => {
        // Verifica si el botón "Ok" fue presionado
...
TipoExaModalsForm.propTypes = {
  onClose: PropTypes.func.isRequired,
};

export default TipoExaModalsForm;
