...
function ExamenModalsUpdate(idExamen,{  onClose }) {
  ...
  useLayoutEffect(() => {
    const cargarDatosExamen = async () => {
      if (idExamen) {
        try {
          const respuesta = await axios.get(`http://localhost:5000/api/fcc/examen/${idExamen.idExamen}`);
  ...
  const enviarDatos = async () => {
    ...
    try {
      await axios
        .put(`http://localhost:5000/api/fcc/examen/${idExamen.idExamen}`, dataParaEnviar)
      // Muestra un mensaje de éxito
      Swal.fire({
        title: "¡Éxito!",
        text: "Examen modificado correctamente",
        icon: "success",
      ...
      Swal.fire({
        title: "Error",
        text: "Hubo un problema al modificar el examen",
        icon: "error",
        confirmButtonText: "Ok",
      }).then((result) => {
        // Verifica si el botón "Ok" fue presionado
...
ExamenModalsUpdate.propTypes = {
  onClose: PropTypes.func.isRequired,
};

export default ExamenModalsUpdate;
